      ...
      <section>
        <div className="grid gap-8 grid-cols-1 sm:grid-cols-2 lg:grid-cols-3">
          {posts.slice(1).map((post) => (
            <article
              key={post.id}
              className="bg-white rounded-lg border border-transparent shadow-md overflow-hidden transition-all duration-300 hover:scale-105 hover:shadow-lg hover:border-blue-500"
            >
              <Image
                src={post.image}
                alt={post.title}
